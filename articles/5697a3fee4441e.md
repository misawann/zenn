...
機械学習と物理のトピックとして、自己学習モンテカルロ法について書いてみます。
実装は以下で公開しています。
...
今回は、Liu et al. (2016) や Fujita et al. (2017) で提案された機械学習による有効模型の構築手法について紹介します。
...
ここで、学習データは固有状態 $\psi_n$ と対応するエネルギー固有値 $E_n$ のペアからなります。この状態に対応する $H_{\mathrm{trial}}$ の固有値を $E'_n$ と、損失関数を $L(E'_n)$ とすると、$L$ を最小化するような
$c_i$ を求めることが目的となります。
...
を利用します。
パラメータ $c_i$ の更新手順として、簡単のため勾配降下法による最適化を示します。$\alpha$を学習率とすると、
...
Ising 模型では、$H_{\mathrm{trial}} = - \sum_i h_i \sigma_i$として、上の計算スキームに従って $h_i$ を更新することになります。

## 実装

今回は、2 次元 Ising 模型に対して適用してみます。Fujita et al. (2017) のように、Hubbard 模型といったより複雑な模型に対しても適用可能ですが、敢えて最も簡単な例を扱います。

実装には Pytorch を使用しました。

まず、原型の Hamiltonian を定義します。
...
ちなみに、[`torch.einsum`](https://pytorch.org/docs/stable/generated/torch.einsum.html)は、Einstein の縮約記法で、第一引数に渡した shape の推移に従って計算してくれる便利関数です。
...
[^2]: 遅ればせながら執筆時に Github Copilot を取り入れてみました。この自然文のほとんどは Copilot による提案を採用しています。平均場の Hamiltonian というフレーズは数十行前にあるので、幅広い文脈を理解していそうです。ドキュメントを書くときにも便利そうですね。恐るべし。
...
まとめると以下のようになります。
https://github.com/misawann/self-learningMC/blob/main/src/optimize.py
...
また、今回は、有効模型の形をある程度仮定することで、その係数を予測するという簡単なタスクに落とし込むことができました。しかし、理想的にはそういう仮定も外したいところです。
...
- [Zenn の執筆を支援する VSCode 拡張 Zenn Editor](https://zenn.dev/negokaz/articles/aa4e12b76d516597a00e)
  ![zenn-editor](https://storage.googleapis.com/zenn-user-upload/82f5b893a221-20221210.jpeg)
