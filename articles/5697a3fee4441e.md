...
物理では、解析が難しい模型の代わりに、自由度が小さく、あるエネルギーの範囲では一致する模型（有効模型）を扱いたいことがしばしばあります。
...
次に、学習や評価を実行する関数です。
`loop` は計算スキームで示した内容です。誤差逆伝播で勾配を計算しパラメータを更新しています。
...
当初はこちらも実装するつもりでしたが、既にコンテンツ量が多くなってしまったので、[^3]気が向いたら実装についても書いてみることにします。
